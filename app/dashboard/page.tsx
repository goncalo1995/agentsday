...
function estimateLinkMetrics(product: ViatorProduct) {
  const seed = product.productCode
    .split("")
    .reduce((total, character) => total + character.charCodeAt(0), 0);
  const clicks = 80 + (seed % 520);
  const conversionRate = 2.4 + ((seed % 34) / 10);
  const estimatedEarnings = Math.round(clicks * (conversionRate / 100) * 11);
...
function generateSocialPost(product: ViatorProduct, url: string, angle?: string | null) {
  const price = product.price ? `${product.currency} ${product.price.toFixed(0)}` : "check today's price";

  return `POV: you found a travel experience worth saving.\n\n${product.title} in ${product.destination} feels perfect for ${angle || "a memorable creator trip"}.\n\nWhy I would post it:\n- ${product.rating ? `${product.rating.toFixed(1)} rated` : "Viator-listed"} experience\n- From ${price}\n- Easy to share with followers who want the same trip\n\nLink: ${url}`;
}
...
function normalizeProduct(item: Record<string, unknown>, currency: Currency): ViatorProduct {
  ...
  const pricing =
    ...
    (product.pricing as Record<string, unknown> | undefined);
  const summary = pricing?.summary as Record<string, unknown> | undefined;
  const reviews =
    (item.reviews as Record<string, unknown> | undefined) ??
    (product.reviews as Record<string, unknown> | undefined);
  const duration =
    (item.duration as Record<string, unknown> | undefined) ??
    (product.duration as Record<string, unknown> | undefined);
  const productDestinations =
    (item.destinations as Array<Record<string, unknown>> | undefined) ??
    (product.destinations as Array<Record<string, unknown>> | undefined);
  const tags = [
    ...(((item.tags as Array<Record<string, unknown>> | undefined) ??
      (product.tags as Array<Record<string, unknown>> | undefined))?.map((tag) =>
      String(tag.name ?? tag.tagId ?? ""),
    ) ?? []),
    ...(((item.categories as Array<Record<string, unknown>> | undefined) ??
      (product.categories as Array<Record<string, unknown>> | undefined))?.map((category) =>
      String(category.name ?? ""),
    ) ?? []),
  ].filter(Boolean);

  return {
    id: String(product.productCode ?? product.id ?? product.title),
    title: String(product.title ?? item.title ?? "Untitled Viator experience"),
    destination: String(
      productDestinations?.[0]?.ref ??
        item.location ??
        product.location ??
    ...
    rating:
      Number(product.rating ?? item.rating ?? reviews?.combinedAverageRating ?? reviews?.averageRating) || null,
    reviewCount:
      Number(product.reviewCount ?? item.reviewCount ?? reviews?.totalReviews ?? reviews?.combinedTotalReviews) ||
      null,
    price:
      Number(product.price ?? item.price ?? summary?.fromPrice ?? summary?.price ?? pricing?.fromPrice) || null,
    ...
    productCode: String(product.productCode ?? item.productCode ?? "VIATOR"),
    productUrl: String(
      product.productUrl ??
        item.productUrl ??
        product.webURL ??
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      const parsed = JSON.parse(stored) as Array<Partial<SavedLink>>;
      setSavedLinks(
        parsed.map((link) => ({
          id: link.id || `${link.productTitle}-${Date.now()}`,
          productTitle: link.productTitle || "Saved Viator experience",
          destination: link.destination || "Travel destination",
          generatedUrl: link.generatedUrl || "",
          createdAt: link.createdAt || new Date().toISOString(),
          provider: "Viator",
          clicks: link.clicks ?? 0,
          estimatedEarnings: link.estimatedEarnings ?? 0,
          conversionRate: link.conversionRate ?? 0,
          socialPost: link.socialPost || "",
        })),
      );
    }
  }, []);
  ...
  const generatedSocialPost = useMemo(() => {
    if (!selectedProduct || !generatedUrl) return "";
    return generateSocialPost(selectedProduct, generatedUrl, intent?.contentAngle);
  }, [generatedUrl, intent?.contentAngle, selectedProduct]);
  ...
  const filteredSavedLinks = useMemo(() => {
    ...
    return savedLinks.filter((link) =>
      [link.productTitle, link.destination, link.generatedUrl]
        .join(" ")
        .toLowerCase()
        .includes(query),
    );
  ...
  const savedTotals = useMemo(
    () =>
      savedLinks.reduce(
        (totals, link) => ({
          clicks: totals.clicks + link.clicks,
          earnings: totals.earnings + link.estimatedEarnings,
        }),
        { clicks: 0, earnings: 0 },
      ),
    [savedLinks],
  ...
  async function searchProducts(useDemo = false) {
    setIsSearching(true);
    setError(null);
    setStatus("Polishing your creator idea...");
    ...
    try {
      ...
      if (!useDemo) {
        ...
        if (agentResponse.ok) {
          ...
          if (nextIntent) applyIntent(nextIntent);
          setRequestPreview(agentData.viatorRequest ?? agentData.filters ?? null);
          setStatus("Creator brief ready. Searching Viator...");
        } else {
          nextIntent = createFallbackIntent(prompt, filters);
          applyIntent(nextIntent);
          setStatus("AI helper unavailable. Searching with your visible filters...");
        }
      } else {
        nextIntent = createFallbackIntent(prompt, filters);
        applyIntent(nextIntent);
      ...
      const activeFilters = {
        ...filters,
        destination: nextIntent
          ? [nextIntent.destination.city, nextIntent.destination.country]
              .filter(Boolean)
              .join(", ")
          : filters.destination,
        keywords: nextIntent?.keywords.join(" ") || filters.keywords,
        category: nextIntent?.categories[0] || filters.category,
      ...
      if (useDemo) {
        ...
        setDestinations([{ id: "demo-lisbon", name: "Lisbon", country: "Portugal" }]);
        setStatus("Demo picks loaded. Choose one and make a share-ready link.");
        return;
      }

      const inspirationResponse = await fetch("/api/viator/products/search", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          searchTerm: activeFilters.destination,
      ...
      if (inspirationResponse.ok) {
        ...
        setAttractions(
          ((inspirationData.attractions ?? []) as Array<Record<string, unknown>>)
            .slice(0, 6)
            .map((attraction) => ({
              ...
              destination: String(attraction.destinationName ?? attraction.primaryDestinationName ?? ""),
            })),
        );
        setDestinations(
          ((inspirationData.destinations ?? []) as Array<Record<string, unknown>>)
            .slice(0, 6)
            .map((destination) => ({
              id: String(destination.destinationId ?? destination.id ?? destination.name),
              name: String(destination.name ?? destination.destinationName ?? "Destination"),
              country: String(destination.country ?? destination.countryName ?? ""),
            })),
        );
      }
      ...
      if (!productResponse.ok) {
        ...
        throw new Error(
          errorData?.details ||
            errorData?.error ||
            "Viator sandbox did not return free-text results.",
        );
      }
      ...
      setAttractions(
        ((productData.attractions ?? []) as Array<Record<string, unknown>>)
          .slice(0, 6)
          .map((attraction) => ({
            id: String(attraction.attractionId ?? attraction.id ?? attraction.name),
            name: String(attraction.name ?? attraction.title ?? "Viator attraction"),
      ...
      setDestinations(
        ((productData.destinations ?? []) as Array<Record<string, unknown>>)
          .slice(0, 6)
          .map((destination) => ({
            id: String(destination.destinationId ?? destination.id ?? destination.name),
            name: String(destination.name ?? destination.destinationName ?? "Destination"),
      ...
      setStatus("Picks loaded. Choose an experience and make a share-ready link.");
    } catch (searchError) {
      setProducts([]);
      setAttractions([]);
      setDestinations([]);
  ...
  function saveGeneratedLink(product = selectedProduct, url = generatedUrl) {
    ...
    const metrics = estimateLinkMetrics(product);
    ...
    setSavedLinks((current) => [
      {
        ...
        socialPost: generateSocialPost(product, url, intent?.contentAngle),
      },
      ...current,
    ]);
  }
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Discover, save, and compare creator referral links.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-3 px-4 pt-6 md:grid-cols-4">
        <StatTile
        ...
        <StatTile
          ...
          label="Dummy clicks"
          value={savedTotals.clicks.toLocaleString()}
        />
        <StatTile
          icon={<DollarSign className="size-4" />}
          label="Dummy earnings"
          value={`EUR ${savedTotals.earnings}`}
        />
        <StatTile
          icon={<Bookmark className="size-4" />}
          label="Saved referrals"
          value={savedLinks.length.toString()}
        />
      </section>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          {activeSection === "discover" ? (
            <>
              <section className="rounded-lg border border-border bg-card p-4 md:p-5">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold">What are you posting about?</h2>
                </div>
              ...
              <section className="rounded-lg border border-border bg-card p-4 md:p-5">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold">Creator filters</h2>
                </div>
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <Field label="Follower-friendly price">
              ...
              {status ? (
                <section className="rounded-lg border border-border bg-card p-4">
                  ...
                  <div className="grid gap-2 text-sm text-muted-foreground md:grid-cols-2">
                    <SummaryItem text="Idea turned into a creator brief" done={Boolean(intent)} />
                    <SummaryItem text="Travel vibes extracted" done={Boolean(intent)} />
                    <SummaryItem text="Viator free-text search sent" done={Boolean(requestPreview)} />
                    <SummaryItem text="Linkable picks loaded" done={products.length > 0} />
                  </div>
              ...
              {attractions.length > 0 || destinations.length > 0 ? (
                <section className="rounded-lg border border-border bg-card p-4">
                  <button
                    className="flex w-full items-center justify-between text-left"
                    onClick={() => setIsInspirationOpen((value) => !value)}
                  >
                    <span className="font-semibold">More ideas for this place</span>
                    <ChevronDown
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {products.length
                      ? `${products.length} link-ready picks${averageRating ? ` - ${averageRating.toFixed(1)} avg rating` : ""}`
                      : "No picks yet"}
                  </span>
                </div>
                {isSearching ? (
            ...
            <section className="rounded-lg border border-border bg-card p-4 md:p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-semibold">Saved referrals</h2>
                  <p className="text-sm text-muted-foreground">
                    Filter the links you have ready for posts, stories, and bios.
                  </p>
                </div>
              ...
              <div className="mb-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
                <Input
                  ...
                  placeholder="Search saved links by place, product, or URL"
                />
                <Button variant="outline" onClick={() => setSavedSearch("")}>
                  Clear
                </Button>
              </div>
              {savedLinks.length === 0 ? (
                ...
                <div className="rounded-lg border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
                  No saved referrals match that filter.
                </div>
              ) : (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="mt-3 grid gap-2 text-sm sm:grid-cols-3">
                          <MetricPill label="Provider" value={link.provider} />
                          <MetricPill label="Clicks" value={link.clicks.toLocaleString()} />
                          <MetricPill label="Earned" value={`EUR ${link.estimatedEarnings}`} />
                        </div>
                      </div>
                      ...
                      {link.socialPost ? (
                        <div className="rounded-lg bg-secondary p-3 md:col-span-2">
                          <div className="mb-2 flex items-center justify-between gap-2">
                            <p className="text-sm font-medium">Post draft</p>
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => copyText(link.socialPost, `${link.id}-post`)}
                            >
                              {copied === `${link.id}-post` ? (
        ...
        <aside className="space-y-4">
          <section className="rounded-lg border border-border bg-card p-4">
            <h2 className="mb-3 font-semibold">Creator brief</h2>
            <div className="space-y-3 text-sm">
              <div>
                <p className="text-muted-foreground">Place</p>
                <p className="font-medium">
                  {(intent?.destination.city || filters.destination.split(",")[0] || "Any destination")}
                  {intent?.destination.country ? `, ${intent.destination.country}` : ""}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Content angle</p>
                <p className="font-medium">
                  {intent?.contentAngle || "A helpful travel idea your audience can save"}
                </p>
              </div>
              <div>
                <p className="mb-2 text-muted-foreground">Vibes</p>
                <div className="flex flex-wrap gap-2">
                  {(intent?.keywords.length ? intent.keywords : filters.keywords.split(/\s|,/).filter(Boolean))
                    .slice(0, 6)
                    .map((keyword) => (
          ...
          <section className="rounded-lg border border-border bg-card p-4">
            <h2 className="mb-3 font-semibold">From idea to post</h2>
            <div className="space-y-2 text-sm text-muted-foreground">
              {[
                "Creator idea",
                "Creator brief",
                "Viator free-text search",
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          {selectedProduct ? (
            <div className="space-y-4">
              ...
              <Field label="Social post draft">
                <Textarea readOnly value={generatedSocialPost} className="min-h-40 text-sm" />
              </Field>
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <Button
                  variant="outline"
                  onClick={() => copyText(generatedSocialPost, "modal-post-copy")}
                >
                  {copied === "modal-post-copy" ? (
                    <Check className="mr-2 size-4" />
                  ) : (
